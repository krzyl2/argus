...
- [x] **BACKFILL-01**: A cold entity (no checkpoint, `n_seen == 0`) is primed from InfluxDB history before live streaming, so an entity with sufficient history is warmed up on its first live reading
- [x] **BACKFILL-02**: Backfill is idempotent — an orchestrator restart against an already-primed or checkpointed detector never re-feeds historical data
- [x] **BACKFILL-03**: Backfill degrades safely — InfluxDB unconfigured, unreachable, or returning no rows produces a warning and normal live warm-up, never a startup failure
- [x] **BACKFILL-04**: The same backfill pass primes the orchestrator's `FrozenSensorDetector` rolling window, so frozen-sensor detection is not blind for N readings after a restart
...
| BACKFILL-01 | Phase 15 | Complete |
| BACKFILL-02 | Phase 15 | Complete |
| BACKFILL-03 | Phase 15 | Complete |
| BACKFILL-04 | Phase 15 | Complete |
